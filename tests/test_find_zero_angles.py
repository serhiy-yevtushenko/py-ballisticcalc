...
def find_max_height_robust(scipy_calc, shot_factory):
    ...
    try:
        hit_result = scipy_calc.fire(
            shot, Distance.Meter(1), time_step=0.001, extra_data=True
        )
...
def check_shot_out_of_range(scipy_calc, shot_factory, point_x, point_y):
    ...
    with pytest.raises(OutOfRangeError):
        scipy_calc._engine_instance.find_zero_angle(
            shot, Distance.Meter(distance_in_meters)
        )
    with pytest.raises(OutOfRangeError):
...
def check_shot_result_equivalent(scipy_calc, shot_factory, point_x, point_y):
    ...
    print(
        f"{shot_factory.__name__} - point: {point_x=} {point_y=} {distance>>Distance.Meter=} {look_angle>>Angular.Degree=}"
    )
    shot = shot_factory()
...
def create_7_62_mm_shot():
    ...
    weight = Weight.Grain(180)
    dm = DragModel(
        bc=0.2860, drag_table=TableG1, weight=weight, diameter=diameter, length=length
    )
