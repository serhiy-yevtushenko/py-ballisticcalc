...
def compute_look_angle_and_distance_for_point(point_x_in_meters: float, point_y_in_meters: float):
    ...
    # print(f"{point_x_in_meters=} {point_y_in_meters=} {sight_height_meters=} {shot.cant_angle=}")
    # sight_height_y = math.cos(shot.cant_angle >> Angular.Radian) * sight_height_meters
    # print(f"{point_x_in_meters=} {point_y_in_meters=} {sight_height_y=}")
    #
    # height_diff = point_y_in_meters + sight_height_y
    # print(f"{height_diff=}")

    # look_angle_in_degrees = math.degrees(math.atan2(height_diff
    #                                                 , point_x_in_meters))
    #distance_in_meters = math.sqrt(point_x_in_meters ** 2 + (height_diff ** 2))
...
def create_7_62_mm_shot():
    ...
    gun = Weapon(sight_height=Distance.Millimeter(-100), twist=Distance.Millimeter(300))
    new_shot = Shot(weapon=gun, ammo=ammo)  # Copy the zero properties; NB: Not a deepcopy!
    return new_shot
